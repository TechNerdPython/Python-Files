...
wn.setup(1200, 750)
wn.bgcolor("black")
wn.title("Mass Defense Game by @BandaruLalith")
wn.tracer(0)

# Testing / Start of game values
num_of_defenders = 10
num_of_attackers = 100
num_of_defender_weapons = 10
...
while True:
    ...
    for weapon in defender_weapons:
        if weapon.active == False:
            ...
            # Find enemy to aim it
            attacker = random.choice(attackers)
            weapon.heading = math.atan2(attacker.y - defender.y, attacker.x - defender.x)
            break
    ...
    for weapon in attacker_weapons:
        if weapon.active == False:
            ...
            # Find enemy to aim it
            attacker = random.choice(attackers)
            weapon.heading = math.atan2(defender.y - attacker.y, defender.x - attacker.x)
            break
...
# Check for collisions between enemy weapons and defenders
    for weapon in defender_weapons:
        if weapon.active == True:
            for attacker in attackers:
                if attacker.is_collision(weapon, 12):
